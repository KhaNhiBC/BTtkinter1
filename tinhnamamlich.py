...
list_thiencan =  ['Canh', 'Tân', 'Nhâm', 'Quý', 'Giáp', 'Ất', 'Bính', 'Đinh', 'Mậu', 'Kỷ']
list_diachi =  ['Thân', 'Dậu', 'Tuất', 'Hợi', 'Tí', 'Sửu', 'Dần', 'Mão', 'Thìn', 'Tỵ', 'Ngọ', 'Mùi']
list_hinh =  ['than.gif', 'dau.gif', 'tuat.gif', 'hoi.gif', 'ti.gif', 'suu.gif', 'dan.gif', 'mao.gif',
              'thin.gif', 'ty.gif', 'ngo.gif', 'mui.gif']
def tinhnam():
    nam = int(entryNam.get())
    lblKetQua.set(tinh_can_2(list_thiencan,nam)+''+tinh_chi_2(list_diachi,nam))
    ...
    canvas.gif1 = gif1
    lblHienThiKetQua = tkinter.Label(novi, textvariable=lblKetQua, relief=RAISED, anchor=
    NW)
...
btnTinh = tkinter.Button(master=top, text='Tính', command=tinhnam, bg='yellow')
btnTinh.pack()
top.mainloop()
